...
public class QCraft
{
    ...
    public static class Items
    {
        ...
        public static ItemEOE eoe;
        public static ItemEOO eoo;
        ...
        switch( packet.packetType )
        {
            ...
            case QCraftPacket.SetComputerPortalID:
            {
                ...
                if( entity != null && entity instanceof TileEntityQuantumComputer )
                {
                    ...
                    if( QCraft.canPlayerCreatePortals( entityPlayer ) && !computer.isTeleporterEnergized() )
                    {
                        ...
                        world.notifyBlockUpdate(new BlockPos( x, y, z ), blockState, blockState, 0);
                    }
                }
                break;
            }
            ...
            case QCraftPacket.ConfirmGoToServer:
            {
                ...
                if(entity instanceof TileEntityQuantumComputer)
                {
                    ...
                    if(computer.isTeleporterEnergized() && expectedServer != null && expectedServer.equals(actualServer))
                    {
    ...
    {
        NBTTagCompound signedLuggage = CompressedStreamTools.read(new DataInputStream(new ByteArrayInputStream(signedLuggageData)), NBTSizeTracker.INFINITE);
        byte[] luggageData = signedLuggage.getByteArray("luggage");
        NBTTagCompound luggage = CompressedStreamTools.read(new DataInputStream(new ByteArrayInputStream(luggageData)), NBTSizeTracker.INFINITE);
        ...
        if( signedLuggage.hasKey( "key" ) )
        {
            ...
            else if( !EncryptionRegistry.Instance.getVerifiedPublicKeys().contains(key) )
            {
                ...
                if( forceVerify )
                {
                    ...
                    instigator.sendMessage( new TextComponentString(
            ...
            if( !EncryptionRegistry.Instance.verifyData( key, luggageSignature, luggageData ) )
            {
                ...
                entityPlayer.sendMessage( new TextComponentString( "Portal Link failed:" ) );
                entityPlayer.sendMessage( new TextComponentString( "Signature violation." ) );
            ...
            if( EncryptionRegistry.Instance.getReceivedLuggageIDs().contains( uuid ) )
            {
                ...
                entityPlayer.sendMessage( new TextComponentString( "Portal Link failed:" ) );
                entityPlayer.sendMessage( new TextComponentString( "Luggage duplicate." ) );
                QCraft.requestDiscardLuggage( entityPlayer, signedLuggageData );
                return LuggageVerificationResult.UNTRUSTED;
            }
        ...
        {
            entityPlayer.sendMessage( new TextComponentString( "Portal Link failed:" ) );
            entityPlayer.sendMessage( new TextComponentString( "Signature missing." ) );
            QCraft.requestDiscardLuggage( entityPlayer, signedLuggageData );
            return LuggageVerificationResult.UNTRUSTED;
        }
    }
            ...
            if( verificationResult != LuggageVerificationResult.UNTRUSTED )
            {
                ...
                if( !alreadyTeleported && luggage.hasKey( "destinationPortal" ) )
                {
                        ...
                        (verificationResult == LuggageVerificationResult.LOCALKEY && isDestination) )
                    {
                        ...
                        {
                            entityPlayer.sendMessage( new TextComponentString( "Portal Link failed:" ) );
                            entityPlayer.sendMessage( new TextComponentString( "There is no portal on this server called \"" + destination + "\"" ) );
                        }
    ...
    public static void spawnQuantumDustFX( World world, BlockPos pos )
    {
        proxy.spawnQuantumDustFX( world, pos);
    }
