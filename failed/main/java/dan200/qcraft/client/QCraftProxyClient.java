...
public class QCraftProxyClient extends QCraftProxyCommon
{
    ...
    @Override
    public void spawnQuantumDustFX( World world, BlockPos pos )
    {
        Minecraft mc = Minecraft.getMinecraft();
        double dx = mc.getRenderViewEntity().posX - pos.getX();
        double dy = mc.getRenderViewEntity().posY - pos.getY();
        double dz = mc.getRenderViewEntity().posZ - pos.getZ();
        if( dx * dx + dy * dy + dz * dz < 16.0 * 16.0 )
        {
            Particle fx = new ParticleQuantumDust( world, pos.getX(), pos.getY(), pos.getZ(), 1.0f );
    ...
    @Override
    public World getDefWorld() {
        return Minecraft.getMinecraft().getIntegratedServer().getWorld(0); //gets the client world dim 0 handler
    }
    ...
    {
        ItemStack[] types = ItemQBlock.getTypes( item );
        ItemStack type = cycleType( types );
        if( type.isEmpty() )
        {
    ...
    private ItemStack cycleType( ItemStack[] types )
    {
